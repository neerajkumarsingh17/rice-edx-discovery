...
class MinimalProgramSerializer(DynamicFieldsMixin, BaseModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'is_program_eligible_for_one_click_purchase', 'degree', 'curricula', 'marketing_hook', 'rice_subject_id'
        )
...
class ProgramSerializer(MinimalProgramSerializer):
    ...
    def get_pathway_name(self, obj):
        try:
            pathway = Pathway.objects.get(programs=obj)
            name = pathway.name
        except (Pathway.DoesNotExist, Pathway.MultipleObjectsReturned):
            name = ''
        return name
    ...
    class Meta(MinimalProgramSerializer.Meta):
        ...
        fields = MinimalProgramSerializer.Meta.fields + (
            ...
            'enrollment_count', 'recent_enrollment_count', 'topics', 'credit_value', 'pathway_name'
        )
...
class PathwaySerializer(BaseModelSerializer):
    ...
    @classmethod
    def prefetch_queryset(cls, partner, lms_id=None):
        if lms_id is not None:
            queryset = Pathway.objects.filter(partner=partner, lms_id__in=lms_id.split(","))
        else:
